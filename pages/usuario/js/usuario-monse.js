...
function aplicarFiltros() {
  var inputCargo = document.getElementById("input-cargo");
  var inputLugar = document.getElementById("input-lugar");
  var textoCargo = inputCargo ? inputCargo.value.toLowerCase().trim() : "";
  var textoLugar = inputLugar ? inputLugar.value.toLowerCase().trim() : "";

  var resultado = empleosDB.filter(function(e) {
...
function renderLista(lista) {
  ...
  cont.innerHTML = "";
  if (lista.length === 0) {
    cont.innerHTML = '<div class="alert alert-secondary text-center">No se encontraron empleos con esos criterios.</div>';
    return;
  }
...
function verDetalle(id) {
  var e = null;
  for (var i = 0; i < empleosDB.length; i++) {
    if (empleosDB[i].id === id) { e = empleosDB[i]; break; }
  }
  if (!e) return;
